...
function App() {
  ...
  const wrongGuessCount = guessedArray.filter(
    (wrongLetter) => !currentWord.includes(wrongLetter)
  ).length;
  ...
  const isGameWon = currentWord
    .split("")
    .every((correctLetter) => guessedArray.includes(correctLetter));
  ...
  function keyboardButton(letter: string) {
    setGuessedArray((prevGuess) =>
    ...
    <main className="max-w-150 h-screen mx-auto flex flex-col justify-center items-center space-y-8 font-hanken">
      ...
      <header className="max-w-90 flex flex-col justify-center items-center">
        <div className="text-center">
          <h1 className="text-2xl font-medium text-zinc-300 mb-1">Guess the Word</h1>
          <p className="text-base font-medium text-gray-400 mb-5">
            Guess the word in under 8 attempts to keep the programming world safe from Assembly!
          </p>
          <div className="max-w-full h-15">{isGameOver && <Status isGameWon={isGameWon} />}</div>
        </div>
      </header>
      <div className="max-w-85 h-auto flex flex-wrap justify-center items-center gap-1">
        {displayAttempt}
      </div>
      <div className="flex flex-row gap-1">{displayWordField}</div>
      <div className="max-w-120 flex flex-wrap justify-center items-center gap-2">
        {displayKeyboard}
      </div>
      <div className="w-60 h-10">{isGameOver && <Button newGameButton={newGameButton} />}</div>
    </main>
  );
}
